...
-- subquery to find customers with orders above
SELECT * FROM customers WHERE customer_id 
IN (SELECT customer_id FROM orders GROUP BY
customer_id  HAVING AVG(order_val)>(SELECT AVG(order_val) FROM orders));



SELECT * FROM customers WHERE customer_id 
NOT IN (SELECT customer_id FROM orders GROUP BY
customer_id  HAVING AVG(order_val)>(SELECT AVG(order_val) FROM orders));